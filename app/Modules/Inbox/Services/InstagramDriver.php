...
class InstagramDriver implements ChannelDriverInterface
{
    ...
    private function processInboundMessage(string $pageId, array $event): ?Message
    {
        ...
        if (! empty($attachments)) {
            ...
            $isLikeOrSticker = false;
            foreach ($attachments as $att) {
                if (in_array($att['type'] ?? '', ['like', 'sticker', 'animated_media'], true) || ! empty($att['payload']['sticker_id'])) {
                    $isLikeOrSticker = true;
            ...
            } elseif ($attType === 'share' || $attType === 'story_mention') {
                $type = 'image';
                $msgBody = $msgText ?: 'Shared post/story';
            } else {
                $type = $attType;
