...
class InboxController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $unreadCounts = [
            'all' => (int) Conversation::where('workspace_id', $workspaceId)->where('status', 'open')->sum('unread_count'),
            'mine' => (int) Conversation::where('workspace_id', $workspaceId)->where('status', 'open')->where('assigned_user_id', $userId)->sum('unread_count'),
            'unassigned' => (int) Conversation::where('workspace_id', $workspaceId)->where('status', 'open')->whereNull('assigned_user_id')->sum('unread_count'),
        ];
    ...
    public function reply(Request $request, Conversation $conversation): JsonResponse|RedirectResponse
    {
        ...
        if ($request->hasFile('attachment')) {
            ...
            $localFullPath = null;
            try {
                $localFullPath = $this->storageManager->disk()->path($storedPath);
            } catch (\Throwable) {}

            $msgPayload = array_merge($msgPayload ?? [], array_filter([
                'media_id' => $mediaId,
    ...
    /** Ultra fast lightweight polling endpoint for active conversation messages (JSON) */
    public function poll(Request $request, Conversation $conversation): JsonResponse
    {
        ...
        if ($messages->isNotEmpty() && $conversation->unread_count > 0) {
            $conversation->update(['unread_count' => 0]);
        }
    ...
    /** Ultra fast lightweight polling endpoint for conversation list (JSON) */
    public function pollConversations(Request $request): JsonResponse
    {
        ...
        $payload = $conversations->toArray();
        $payload['unread_counts'] = $unreadCounts;
