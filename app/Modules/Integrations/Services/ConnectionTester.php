...
class ConnectionTester
{
    ...
    private function testMeta(IntegrationConfig $config): array
    {
        ...
        if (!empty($appId) && !empty($appSecret)) {
            $resp = HttpFacade::timeout(10)->get("https://graph.facebook.com/v20.0/{$appId}", [
                'access_token' => "{$appId}|{$appSecret}",
            ]);
            if ($resp->successful() && isset($resp->json()['id'])) {
                $appName = $resp->json()['name'] ?? 'Meta App';
                return ['ok' => true, 'message' => "Connected successfully to {$appName} (App ID: {$appId})"];
            }
            return ['ok' => false, 'message' => $resp->json()['error']['message'] ?? 'Invalid App ID or App Secret.'];
        }

        return ['ok' => false, 'message' => 'Please provide App ID & App Secret or a System User Token.'];
    }
    ...
    private function testLlm(IntegrationConfig $config): array
    {
        ...
        if (str_contains($config->provider, 'openai')) {
            $model = !empty($creds['default_model']) ? $creds['default_model'] : 'gpt-4o-mini';
        ...
        if (str_contains($config->provider, 'anthropic')) {
            $model = !empty($creds['default_model']) ? $creds['default_model'] : 'claude-3-haiku-20240307';
            $resp = HttpFacade::timeout(15)
            ...
            return $resp->successful()
                ? ['ok' => true,  'message' => "Anthropic connection successful (Model: {$model})."]
        ...
        if (str_contains($config->provider, 'nvidia')) {
            $baseUrl = rtrim($creds['base_url'] ?? 'https://integrate.api.nvidia.com/v1', '/');
            $model = !empty($creds['default_model']) ? $creds['default_model'] : 'openai/gpt-oss-20b';
            $resp = HttpFacade::timeout(15)
                ->withToken($apiKey)
            ...
            return $resp->successful()
                ? ['ok' => true,  'message' => "NVIDIA NIM connection successful (Model: {$model})."]
                : ['ok' => false, 'message' => $resp->json()['error']['message'] ?? $resp->json()['detail'] ?? 'NVIDIA NIM error.'];
        }
