...
class AnalyticsService
{
    ...
    {
        $driver = DB::connection()->getDriverName();
        $diffExpr = fn ($col1, $col2) => $driver === 'sqlite'
            ? "(unixepoch({$col2}) - unixepoch({$col1}))"
            : "TIMESTAMPDIFF(SECOND, {$col1}, {$col2})";

        $row = CampaignRecipient::where('campaign_id', $campaignId)
            ->selectRaw("
                AVG(CASE WHEN sent_at IS NOT NULL AND delivered_at IS NOT NULL
                    THEN {$diffExpr('sent_at', 'delivered_at')} END) as sent_to_delivered,
                AVG(CASE WHEN delivered_at IS NOT NULL AND read_at IS NOT NULL
                    THEN {$diffExpr('delivered_at', 'read_at')} END) as delivered_to_read,
                AVG(CASE WHEN sent_at IS NOT NULL AND read_at IS NOT NULL
                    THEN {$diffExpr('sent_at', 'read_at')} END) as sent_to_read
            ")
            ->first();
    ...
    public function agentLeaderboard(int $wsId, Carbon $from, Carbon $to): array
    {
        ...
        $driver = DB::connection()->getDriverName();
        $diffMinExpr = $driver === 'sqlite'
            ? "(unixepoch(first_out.sent_at) - unixepoch(first_in.created_at)) / 60.0"
            : "TIMESTAMPDIFF(MINUTE, first_in.created_at, first_out.sent_at)";

        // First-response time: earliest outbound message after first inbound, per conversation
        $firstResponse = DB::select("
            SELECT
                c.assigned_user_id,
                AVG({$diffMinExpr}) AS avg_first_response_min
    ...
    public function newClientsPerWeek(int $weeks = 12): array
    {
        ...
        $rows = Client::selectRaw("{$dateExpr} as week, COUNT(*) as clients")
    ...
    {
        $driver = DB::connection()->getDriverName();
        $diffSecExpr = $driver === 'sqlite'
            ? "(unixepoch({$endField}) - unixepoch({$startField}))"
            : "TIMESTAMPDIFF(SECOND, {$startField}, {$endField})";

        $rows = Conversation::where('workspace_id', $wsId)
            ->whereNotNull($endField)
            ->whereBetween($endField, [$from->startOfDay(), $to->endOfDay()])
            ->selectRaw("DATE({$endField}) as date, {$diffSecExpr} as seconds")
            ->get();
