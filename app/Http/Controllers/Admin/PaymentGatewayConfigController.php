...
class PaymentGatewayConfigController extends Controller
{
    ...
    private const GATEWAYS = ['stripe', 'paypal', 'paddle', 'razorpay', 'cashfree', 'tap', 'paystack', 'paymob', 'myfatoorah', 'xendit', 'mollie', 'square', 'mercadopago', 'manual'];
    ...
    private const LABELS = [
        ...
        'manual' => 'Manual Payment (bKash, Nagad, Bank, Crypto)',
    ];
    ...
    public function index(): Response
    {
        ...
        foreach ($gateways as $key) {
            ...
            $isConfigured = $key === 'manual'
                ? \App\Models\ManualPaymentMethod::where('enabled', true)->exists()
                : ($config?->hasActiveCredentials() ?? false);

            $list[] = [
                'gateway' => $key,
                'name' => self::LABELS[$key] ?? ucfirst($key),
                'enabled' => $key === 'manual' ? ($config?->enabled ?? true) : ($config?->enabled ?? false),
        ...
        return Inertia::render('Admin/PaymentGateways/Index', [
            'gateways' => $list,
            'pending_manual_count' => \App\Models\ManualPaymentRequest::where('status', 'pending')->count(),
        ]);
    }
    ...
    public function update(Request $request, string $gateway): RedirectResponse
    {
        ...
        if (! empty($validated['enabled']) && $gateway !== 'manual') {
