...
<html lang="{{ str_replace('_', '-', $locale) }}" dir="{{ $htmlDir }}">
    <head>
        ...
        <title inertia>{{ config('app.name', 'Omni agent') }}</title>
        @php
            try {
        ...
        @else
            {{-- Fallback brand icon: SVG for modern browsers, .ico for legacy,
                 PNG apple-touch for iOS home-screen. See public/omni_agent-icon.svg. --}}
            <link rel="icon" type="image/svg+xml" href="/omni_agent-icon.svg">
