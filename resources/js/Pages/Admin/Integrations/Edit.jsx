...
const SETUP_GUIDES = {
    ...
    llm_nvidia_default: {
        title: 'NVIDIA NIM API Setup',
        steps: [
            'Go to build.nvidia.com and sign in with your NVIDIA account.',
            'Navigate to API Keys → Generate Key (starts with nvapi-...).',
            'Select your desired model (e.g. openai/gpt-oss-20b or meta/llama-3.1-70b-instruct).',
            'Paste your API key and choose or enter your default model below.',
        ],
        link: 'https://build.nvidia.com',
        linkLabel: 'Open NVIDIA NIM Portal',
    },
};
...
const BRAND = {
    ...
    llm_nvidia_default:      { bg: 'bg-emerald-100 dark:bg-emerald-900/30', color: '#76B900', icon: <svg viewBox="0 0 24 24" fill="currentColor" className="h-5 w-5"><path d="M3.562 7.375c4.71-4.708 12.348-4.708 17.058 0 4.316 4.315 4.672 11.11.848 15.827-.417.514-1.18.57-1.666.12-.46-.425-.503-1.144-.105-1.635 3.123-3.856 2.836-9.406-.688-12.93-3.844-3.844-10.076-3.844-13.92 0-3.484 3.483-3.81 8.93-.81 12.793.415.534.33 1.298-.188 1.724-.508.417-1.25.337-1.678-.207-3.66-4.71-3.268-11.378.969-15.612zM8.3 11.12c2.62-2.618 6.865-2.618 9.483 0 2.392 2.392 2.593 6.16.48 8.78-.415.513-1.173.57-1.657.126-.46-.423-.506-1.14-.112-1.632 1.488-1.846 1.348-4.502-.336-6.185-1.99-1.99-5.215-1.99-7.204 0-1.802 1.802-1.966 4.622-.42 6.61.41.53.33 1.288-.184 1.714-.504.416-1.242.34-1.673-.2-2.023-2.607-1.808-6.3-0.377-8.213zM12 13.95c.578 0 1.05.472 1.05 1.05s-.472 1.05-1.05 1.05-1.05-.472-1.05-1.05.472-1.05 1.05-1.05z"/></svg> },
...
function SelectField({ label, fieldKey, value, onChange, required, hint, options = [] }) {
    ...
    const [useCustom, setUseCustom] = useState(Boolean(isCustom));
    ...
    return (
        <div className="space-y-1.5">
            ...
            <select
                value={useCustom ? '__custom__' : (value || (options[0]?.value ?? ''))}
            ...
            {useCustom && (
                <input
                    ...
                    value={value || ''}
                    onChange={(e) => onChange(fieldKey, e.target.value)}
                    placeholder="Enter custom model (e.g. openai/gpt-oss-20b)"
                    className="w-full rounded-lg border border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-800 px-3 py-2 text-sm text-neutral-900 dark:text-neutral-100 focus:outline-none focus:ring-2 focus:ring-brand-500"
                />
            )}
