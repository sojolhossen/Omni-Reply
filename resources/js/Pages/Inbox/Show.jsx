...
function galleryImageSrc(msg, conversationId) {
    ...
    return p.preview_url
        ?? p.image?.preview_url
        ?? p.sticker?.preview_url
        ?? p.attachments?.[0]?.payload?.url
        ?? p.attachments?.[0]?.url
        ?? p.message?.attachments?.[0]?.payload?.url
        ?? p.message?.attachments?.[0]?.url
        ?? p.url
...
function StickerBubble({ src, conversationId, messageId, body }) {
    const [imgErr, setImgErr] = useState(false);
    const proxyUrl = src ?? (messageId ? route('client.inbox.message-media', { conversation: conversationId, message: messageId }) : null);

    if (imgErr || !proxyUrl) {
        return (
            <div className="p-1 text-5xl select-none flex items-center justify-center" title="Sticker">
                {body && body !== 'Sticker' && body !== '(media)' ? body : '👍'}
            </div>
        );
    }
...
function MessageBubble({ msg, conversationId }) {
    ...
    const previewUrl  = p.preview_url
        ?? p[mediaType]?.preview_url
        ?? p.image?.preview_url
        ?? p.sticker?.preview_url
        ?? p.audio?.preview_url
        ?? p.video?.preview_url
        ?? p.document?.preview_url
        ?? p.attachments?.[0]?.payload?.url
        ?? p.attachments?.[0]?.url
        ?? p.message?.attachments?.[0]?.payload?.url
        ?? p.message?.attachments?.[1]?.payload?.url
        ?? p.message?.attachments?.[0]?.url
        ?? p.url
        ?? p.link
        ?? null;
    ...
    const isLikeSticker = msg.body === '👍' || p.sticker_id === 369239263222822 || p.attachments?.[0]?.payload?.sticker_id === 369239263222822;
    const sticker  = mediaType === 'sticker' || (mediaType === 'image' && isLikeSticker);
    const caption  = p.caption ?? p[mediaType]?.caption ?? (msg.body && msg.body !== '(media)' && msg.body !== '👍' ? msg.body : '');
    ...
    if (sticker) {
        return (
            <div className={`flex ${isOut ? 'justify-end' : 'justify-start'} mb-2`}>
                <StickerBubble src={mediaSrc} conversationId={conversationId} messageId={msg.id} body={msg.body} />
            </div>
        );
    }
        ...
        <div className={`flex ${isOut ? 'justify-end' : 'justify-start'} mb-2`}>
            <div className={bubbleBase}>
                ...
                <div className="px-3 py-2.5">
                    ...
                    {/* TEXT / fallback */}
                    {!templateComponents && (mediaType === 'text' || (!['image','video','audio','document','location','contacts','interactive','template','poll','event','unsupported','sticker'].includes(mediaType))) && (
                        <WaText text={msg.body || (isLikeSticker ? '👍' : '(media)')} />
                    )}
...
function formatLastMsgPreview(lastMsg) {
    if (!lastMsg) return '';
    if (lastMsg.body && lastMsg.body !== '(media)') return lastMsg.body;
    if (lastMsg.type === 'sticker') return '👍';
        ...
        <Link
            ...
            className={`block px-3 py-3 border-b border-neutral-100 dark:border-neutral-800 transition-colors ${
                isActive
                    ...
                    : unreadCount > 0
                        ? 'bg-brand-50/20 dark:bg-brand-950/20 hover:bg-brand-50/40 dark:hover:bg-brand-950/40'
                ...
                <button onClick={handleContactClick} title={t('inbox.view_contact')} className="relative shrink-0 group">
                    <div className={`h-9 w-9 rounded-full flex items-center justify-center text-sm font-semibold transition ${
                        unreadCount > 0
                            ...
                            : 'bg-brand-100 dark:bg-brand-900/30 text-brand-700 dark:text-brand-300 group-hover:ring-2 group-hover:ring-brand-400'
                    }`}>
                        {name[0]?.toUpperCase() ?? '?'}
                    </div>
                    <span className="absolute -bottom-0.5 -right-0.5 h-4 w-4 rounded-full bg-white dark:bg-neutral-900 flex items-center justify-center shadow-xs">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-1">
                        <button
                            ...
                            className={`text-sm truncate text-left hover:underline ${unreadCount > 0 ? 'font-bold text-neutral-950 dark:text-white' : 'font-medium text-neutral-700 dark:text-neutral-300'}`}
                            title={t('inbox.view_contact_profile')}
                        >
                            {name}
                        </button>
                        <span className={`text-[11px] shrink-0 ${unreadCount > 0 ? 'font-semibold text-brand-600 dark:text-brand-400' : 'text-neutral-400'}`}>
                            {conv.last_message_at ? formatTimeTz(conv.last_message_at, userTz) : ''}
                        </span>
                    </div>
                    <div className="flex items-center justify-between gap-1.5 mt-0.5">
                        <p className={`text-xs truncate flex-1 ${unreadCount > 0 ? 'font-semibold text-neutral-900 dark:text-neutral-100' : 'text-neutral-400'}`}>
                            {formatLastMsgPreview(conv.last_message)}
                        </p>
                        {unreadCount > 0 && (
                            <span className="shrink-0 min-w-[20px] h-5 px-1.5 rounded-full bg-brand-600 text-white text-[11px] font-bold flex items-center justify-center shadow-sm">
...
function FilterSidebar({ filters, labels, channelAccounts = [], unreadCounts = {}, onFolder, onChannel, onAccount, onLabel }) {
    const { t } = useTranslation();
    return (
        <div className="flex flex-col h-full overflow-y-auto">
            <div className="p-2 border-b border-neutral-100 dark:border-neutral-800">
                ...
                {FOLDERS.map(({ key, labelKey, icon: Icon }) => {
                    const count = key === null ? unreadCounts?.all : key === 'mine' ? unreadCounts?.mine : key === 'unassigned' ? unreadCounts?.unassigned : 0;
                    return (
                        <button key={key ?? 'all'} onClick={() => onFolder(key)}
                            ...
                            {Number(count) > 0 && (
                                <span className="shrink-0 min-w-[18px] h-4.5 px-1.5 text-[10px] font-bold rounded-full bg-brand-600 text-white flex items-center justify-center shadow-xs">
                                    {count > 99 ? '99+' : count}
                                </span>
                            )}
                        </button>
...
export default function InboxShow({
    ...
    const [unreadCounts, setUnreadCounts]   = useState(initialConversations?.unread_counts ?? initialUnreadCounts);
    ...
    useEffect(() => {
        ...
        if (initialConversations?.unread_counts) {
            setUnreadCounts(initialConversations.unread_counts);
        }
    ...
    // High-speed real-time polling (1s) for instant message delivery without reloading
    ...
    useEffect(() => {
        ...
        const checkNewMessages = async () => {
            if (document.visibilityState !== 'visible' || isCancelled) return;

            const currentMsgs = messagesRef.current;
            const maxId = currentMsgs.reduce((max, m) => (m.id > max ? m.id : max), 0);
            ...
            try {
                ...
                const newItems = res.data?.messages ?? [];
                if (newItems.length > 0) {
                    setMessages(prev => {
                        ...
                        uniqueNew.forEach(m => {
                            if (m.direction === 'in') {
                                playInboundSound(m.channel ?? channel);
                            }
                        });
    ...
    }, [conversation.uuid, channel]);

    // High-speed real-time polling (1.5s) for instant conversation sidebar updates
    useEffect(() => {
        let isCancelled = false;

        const checkConversationList = async () => {
            if (document.visibilityState !== 'visible' || isCancelled) return;

            try {
                const res = await axios.get(route('client.inbox.poll-list'), {
                    params: filters,
                });
        ...
        const interval = setInterval(checkConversationList, 1500);
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <aside className="w-48 shrink-0 border-r border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 flex flex-col overflow-hidden">
                    ...
                    <FilterSidebar
                        ...
                        unreadCounts={conversations?.unread_counts ?? unreadCounts}
